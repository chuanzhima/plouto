...
public class PrepaidCardValidator {
    ...
    public Map<Boolean, List<String>> isBindCard(String custId) {

        final String userId = userInfoManager.getUserIdByCustId(custId);
        if (StringUtil.isEmpty(userId)) {
            return new HashMap<Boolean, List<String>>() {{
                put(Boolean.FALSE, Arrays.asList(ResultCodeConstant.USER_MAPPING_NOT_EXIST));
            }};
        }
        ...
        if (null == cardInfoEntity) {
            return new HashMap<Boolean, List<String>>() {{
                put(Boolean.FALSE, Arrays.asList(ResultCodeConstant.USER_NOT_BIND_CARD));
            }};
        }
        if (!AccountBizConstant.PREPAID_CARD_INFO_STATUS_NORMAL.equals(cardInfoEntity.getStatus())) {
            return new HashMap<Boolean, List<String>>() {{
                put(Boolean.FALSE, Arrays.asList(ResultCodeConstant.CARD_STATUS_INVALID));
            }};
        }

        return new HashMap<Boolean, List<String>>() {{
            put(Boolean.TRUE, Arrays.asList(cardInfoEntity.getPrepaidCardNo(), userId));
        }};
    }
