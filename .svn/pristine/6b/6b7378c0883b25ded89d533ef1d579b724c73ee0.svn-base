 ...
 * Date 2016/11/15 11:38
 */
@Component(value = "consumeValidator")
public class ConsumeValiator {

    private static final Logger LOGGER = LoggerFactory.getLogger(ConsumeValiator.class);
    @Autowired
    ...
    public Map<Boolean, String> validate(RequestHeader header, AccountTypeEnum typeEnum, String typeValue, BigDecimal
        ...
        Boolean isRepeat = transValidator.isRepeatTrans(header.getReqSeq(), header.getTxDate
                (), header.getTxTime(), AccountTransTypeEnum.CONSUME.key(), AccountInfoStatusEnum
                .NORMAL.value());
        if (isRepeat) {
        ...
        // 单日，单笔，累计限额
        final String limitResult = transValidator.transLimits(accountLevel, accountId, AccountTransTypeEnum.RECHARGE.key(),
                amount);
